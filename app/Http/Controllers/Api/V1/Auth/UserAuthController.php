...
class UserAuthController extends Controller
{
    ...
    /**
     * Display the authenticated user's history of organization activations
     * (both paid and promo).
     * Route: GET /user/organization-activation-history
     */
    public function organizationActivationHistory(Request $request)
    {
        /** @var \App\Models\User $user */
        $user = $request->user();

        // Fetch Paid Activations (where this user created/paid for an organization)
        $paidActivations = Payment::where('user_id', $user->id)
            ->whereNotNull('organization_id') // Ensure it's an org payment
            ->where('status', 'succeeded')    // Only successful payments
            ->with('organization:id,name,organization_code')
            ->select('id as record_id', 'organization_id', 'amount', 'currency', 'paid_at as activated_at', DB::raw("'paid' as activation_type"))
            ->orderBy('paid_at', 'desc')
            ->get();

        // Fetch Promo Activations (where this user redeemed a promo to create/activate an organization)
        $promoActivations = PromoCodeRedemption::where('user_id', $user->id)
            ->whereNotNull('organization_id')
            ->with(['organization:id,name,organization_code', 'promoCode:id,code'])
            ->select('id as record_id', 'organization_id', 'promo_code_id', 'redeemed_at as activated_at', DB::raw("'promo' as activation_type"))
            ->orderBy('redeemed_at', 'desc')
            ->get();

        // Combine and sort the results
        // We need to ensure 'amount' and 'currency' are present or null for promo activations for consistent mapping
        $combinedHistory = $paidActivations->map(function ($item) {
            // Payment model's 'amount' accessor already converts to dollars
            $item->amount_display = $item->amount; // Use the accessor's output
            return $item;
        })->concat(
            $promoActivations->map(function ($item) {
                $item->amount_display = null; // Promos have no direct monetary amount in this context
                $item->currency = null;
                return $item;
            })
        )->sortByDesc('activated_at')->values(); // Sort combined results by activation date

        // Manual Pagination (if needed for combined results)
        $perPage = $request->input('per_page', 15);
        $currentPage = $request->input('page', 1);
        $currentPageItems = $combinedHistory->slice(($currentPage - 1) * $perPage, $perPage)->values();
